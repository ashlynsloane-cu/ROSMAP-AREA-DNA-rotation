...
PNG = OUTDIR / "weighted_area_encoding_comparison_direct_paper.png"
PDF = OUTDIR / "weighted_area_encoding_comparison_direct_paper.pdf"
...
def main():
    ...
    # Compact shared colorbars for the two heatmap columns.
    cbar_ax1 = fig.add_axes([0.315, 0.185, 0.010, 0.18])
    cb1 = fig.colorbar(corr_images[0], cax=cbar_ax1)
    cb1.set_label("Spearman ρ", fontsize=5.5)
    cb1.ax.tick_params(labelsize=5.0)

    cbar_ax2 = fig.add_axes([0.560, 0.185, 0.010, 0.18])
    cb2 = fig.colorbar(jac_images[0], cax=cbar_ax2)
    cb2.set_label("Jaccard", fontsize=5.5)
    cb2.ax.tick_params(labelsize=5.0)
    ...
    fig.subplots_adjust(top=0.86, bottom=0.17, left=0.10, right=0.97)
