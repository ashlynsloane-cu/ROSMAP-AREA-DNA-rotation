...
BASENAME = OUTDIR / "weighted_area_encoding_tree_final"
...
def draw_scale(ax, x0, x1, y, fractions, labels):
    ...
    for px, lab in zip(xs, labels):
        ...
        ax.text(px, y - 0.014, lab, ha="center", va="top", fontsize=6.2, color=TEXT, zorder=4)
...
def encoding_box(ax, box, *, title, subtitle, representation, examples, mode):
    ...
    ax.text(left, y + h * 0.355, "Examples",
            ha="left", va="top", fontsize=6.4,
            fontweight="bold", color=TEXT, zorder=3)
    ax.text(left, y + h * 0.270, examples,
...
def main():
    ...
    merge(ax, centers, panel_y, 0.288, bottom_cx, bottom[1] + bottom[3])
