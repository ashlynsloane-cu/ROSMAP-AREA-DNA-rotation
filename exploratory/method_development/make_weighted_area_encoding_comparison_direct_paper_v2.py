...
PNG = OUTDIR / "weighted_area_encoding_comparison_direct_paper_v2.png"
PDF = OUTDIR / "weighted_area_encoding_comparison_direct_paper_v2.pdf"
...
def main():
    ...
    gs = fig.add_gridspec(
        2,
        5,
        width_ratios=[1.0, 0.055, 1.0, 0.055, 1.22],
    ...
    for row_idx, family in enumerate(
        ...
        ax_corr = fig.add_subplot(gs[row_idx, 0])
        ax_jac = fig.add_subplot(gs[row_idx, 2])
    ...
    # One shared colorbar per heatmap metric, spanning both rows.
    cax_corr = fig.add_subplot(gs[:, 1])
    cb1 = fig.colorbar(corr_images[0], cax=cax_corr)
    cb1.set_label("Spearman ρ", fontsize=5.7, labelpad=4)
    cb1.ax.tick_params(labelsize=5.0, length=2)

    cax_jac = fig.add_subplot(gs[:, 3])
    cb2 = fig.colorbar(jac_images[0], cax=cax_jac)
    cb2.set_label("Jaccard similarity", fontsize=5.7, labelpad=4)
    cb2.ax.tick_params(labelsize=5.0, length=2)
    ...
    fig.subplots_adjust(top=0.86, bottom=0.18, left=0.105, right=0.975)
