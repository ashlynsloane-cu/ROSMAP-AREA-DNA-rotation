...
DESEQ2_COLOR = "#4C78A8"
REGULAR_COLOR = "#F58518"
WEIGHTED_COLOR = "#54A24B"
PAIRWISE_TEXT_COLOR = "#222222"
...
def parse_args():
    ...
    p.add_argument(
        "--title",
        default=(
            "Complementary Mean- and Rank-Based Gene Signatures in Cognitive AD"
        ),
    )
...
def draw_venn(
    ...
    circles = [
        Circle(
            left,
            r,
            facecolor=DESEQ2_COLOR,
            edgecolor=DESEQ2_COLOR,
            alpha=0.28,
            linewidth=2.5,
        ),
        Circle(
        ...
        Circle(
            top,
            r,
            facecolor=WEIGHTED_COLOR,
            edgecolor=WEIGHTED_COLOR,
            alpha=0.28,
            linewidth=2.5,
        ),
    ]
    ...
    region_text_colors = {
        "DESeq2 only": DESEQ2_COLOR,
        "Adjusted Regular AREA only": REGULAR_COLOR,
        "Weighted AREA only": WEIGHTED_COLOR,
        "DESeq2 + Adjusted Regular AREA only": PAIRWISE_TEXT_COLOR,
        "DESeq2 + Weighted AREA only": PAIRWISE_TEXT_COLOR,
        "Adjusted Regular AREA + Weighted AREA only": PAIRWISE_TEXT_COLOR,
        "All three": "#000000",
    }
    ...
    ax.text(
        ...
        -0.09,
        (
            "All analyses control age_death, sex, RIN, PMI, and sequencing batch. "
            "DESeq2 and Regular AREA use the same 418 AD4-vs-NCI1 participants; "
            "Weighted AREA uses the broader graded NCI→MCI→AD cohort (n=619). "
            "Twelve persistent DESeq2 non-converged genes were excluded from all "
            "methods before BH recomputation. Circle areas are not proportional "
            "to region counts."
        ),
        transform=ax.transAxes,
        ha="center",
...
def main():
    ...
    key_region_file = (
        root + "_regular_weighted_only_not_deseq2.csv"
    )

    key_region_df = comparison.loc[
        (~comparison["DESeq2_significant"])
        & (comparison["Regular_AREA_significant"])
        & (comparison["Weighted_AREA_significant"])
    ].copy()

    key_region_df.to_csv(
        key_region_file,
        index=False,
    )
    ...
    print(f"  Key region: {key_region_file}")
